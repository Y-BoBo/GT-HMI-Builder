...
//	WDT_Start(WDT);
	...
	while (1) {
		...
		swm_delay_ms(1);
//		WDT_Feed(WDT);
		if(uart_status)//´®¿ÚÊÇ·ñ½ÓÊÕÊý¾Ý
		{
			uart_status=0;
			Refresh_New_Page(UART_BUF);	//¸üÐÂÒ³Ãæ
			for(int i=0;i<8;i++)
				printf("%#x ",UART_BUF[i]);
			printf("\r\n");
			memset(UART_BUF,0,8);//Çå¿Õ»º´æÇø
		}
		if(update_sreen_status)//ÆÁÄ»½çÃæÊÇ·ñÓÐ±ä»¯
		{
			update_sreen_status=0;
			send_pack();//·¢ËÍ±ä»¯ºóµÄÊýÖµ
		}
#if _RUNNING_PRINT
		if (_cnt >= 1000) {
			_cnt = 0;
